...
| Вопрос | Ответ |
|---|---|
| `[исключение]` | Инвентаризация всех чтений полей чужой ошибки в `toErrorEvent`/`maskCause`/`mergeContext` — не только тех, что были явно названы, а по правилу «читаешь чужое поле или перечисляешь чужие ключи — оборачивай»: сбой самого перечисления ключей (`Object.keys`) — отдельный риск, не «чтение значения», обычный try/catch на строку цикла его не ловит — закрыт в `mergeContext` (обе стороны мерджа), `freezeEvent` (свой try/catch на весь `context`, отдельно на весь `cause`), `maskContextValue` (её собственный `Object.keys`). Чтение конкретного поля: `name`/`message`/`stack`/`errors`/`cause.cause` у `cause`; `message`/`stack`/`cause`/`code`/`severity`/`context` у верхнеуровневой `AppError`; `code`/`severity`/`context` у вложенной `AppError` внутри `maskCause` — каждое через свой `safeRead`, каждое проверено мутационно ОТДЕЛЬНО (общий тест на несколько полей не покажет, какая из обёрток перестала работать). Гранулярность деградации сознательно выровнена между уровнями: `mergeContext` теряет только испорченную половину мерджа, вложенная `AppError` в `cause` — только испорченное поле, а не всю причину целиком. Аварийная ветка `toErrorEvent` не переиспользует входной (потенциально отравленный) `context` повторно, а собирает событие с заведомо безопасным `{}`. Третья категория, сознательно без индивидуальных ловушек: броски на операциях с коллекциями и на приведении к строке внутри `maskContextValue` (`.entries()`/`.values()`/`.size`/`.slice`/`Object.keys`) и `.href`/`.byteLength` — деградируют до уровня родителя (поля или половины события), не до себя; заводить отдельный `catch` на каждую операцию — пока не окупается, если это изменится, сюда же добавить. Открыто: броски на самих операторах (`'cause' in cause`, `instanceof Error/AggregateError/AppError`) не обёрнуты — цена не потеря поля, а деградация ВСЕГО события до общего `UNKNOWN_ERROR` (не падение процесса и не утечка) |
...
| `[секрет]` | Токен в `message`/`stack`/цепочке `cause` (глубже `MAX_DEPTH` — обрезается); циклический `cause`/`context` — не зацикливается; плоский/вложенный/массивный `context`; ключ `Authorization` — строка ИЛИ объект скрыты целиком (значения в тестах — заведомо не подходящие ни под один формат `maskSecrets`, чтобы не путать маскировку по имени ключа с маскировкой по формату); `api_key`/`private_key`/`mnemonic`/`seed`/`cookie`/`credential`/`passphrase` — тоже распознаются; `Map` с ключом `'authorization'` — по имени ключа; имя ключа ОБЫЧНОГО объекта, содержащее токен (`{ [`session-${token}`]: ... }`) — тоже маскируется, не только у `Map`; два разных ключа, маскирующихся в одно имя, — схлопываются, последний по порядку побеждает (осознанно, без разрешения коллизий); `URL` с токеном в query — через `href`; `Error` напрямую в `context` — через `maskCause`; объект-не-`Error` в `cause` — как context; системная ошибка (`code`/`errno`/`syscall`/`hostname`) в `cause` — поля скопированы и замаскированы; `AggregateError.errors` — вложенные причины замаскированы; токен на границе `MAX_STRING_LENGTH` — маскируется целиком (маска → обрезка, не наоборот) |
| `[рост]` | Цепочка `cause` из 11 звеньев — остановка на `MAX_DEPTH`, проверено обходом `.cause` с чтением `.message` на каждом уровне (не `JSON.stringify`: `message` копии-объекта — обычное поле, но полагаться на то, что сериализация его покажет, — обходной путь, не доказательство); общая ссылка на один объект одновременно в `cause` и `context` — не цикл; циклическая ссылка ВНУТРИ `context` (не через `cause`); чередование `cause↔context` — обрыв проверен на ТОЧНОЙ позиции, рассчитанной от констант (`MAX_DEPTH=8`, 3 единицы глубины на переход через `Error`), а не по факту наличия любого маркера — иначе тест не отличил бы `[depth exceeded]` на своей позиции от `[budget exceeded]` при гипотетическом сбросе счётчика; массив длиннее `MAX_COLLECTION_ITEMS` — маркер переполнения; `Map`/`Set` длиннее `MAX_COLLECTION_ITEMS` (в т.ч. 200 000 записей) — обход по итератору с ручной остановкой на пределе, а не материализация всей коллекции через `[...collection]` до среза, точный остаток — через `.size`, без досчитывания |
| `[типы]` | `Error` напрямую в `context` — `stack` не enumerable, нужен явный `instanceof Error`; bigint под ключом `token` — сознательно не маскируется (примитив, `SENSITIVE_KEY` совпадает и с безобидными именами вроде `tokenExpiresAt`); `Date`/`Map`/`Set` — не превращаются в `{}`; `Buffer`/`TypedArray` — `[binary N bytes]`; `err.message = 123` и `Object.create(null)` — `toSafeString` не бросает, даёт `'123'`/`'[unprintable]'`, не мусор с ложной гарантией `string` |
...
- ~~Геттеры `name`/`message`/`stack`/`errors`, бросающие при чтении, у `cause`, и `message`/
  `stack` у верхнеуровневой ошибки в `toErrorEvent`~~ — исправлено при ревью шага 2 (4a): все
  читаются через общий хелпер `safeRead`, деградируют локально в `'[unreadable]'`, не роняют
  событие целиком.
- Операторы `'cause' in cause` и `instanceof Error/AggregateError/AppError` на самом `cause` не
  обёрнуты — для экзотических объектов (`Proxy` с бросающим `has`/`getPrototypeOf`) это всё ещё
  может бросить. Цена, если это случится, — НЕ потеря отдельного поля `name`, а деградация
  ВСЕГО события до общего `UNKNOWN_ERROR` (`toErrorEvent` перехватывает на верхнем уровне) — не
  падение процесса и не утечка секрета.
...
- Синтетический ключ `'__truncated__'` (объект с числом полей больше `MAX_COLLECTION_ITEMS`)
  может совпасть с настоящим полем объекта с тем же именем и затереть его — уникальность имени
  маркера не гарантирована.
